...
// PDO
$stmt = ("SELECT * FROM coordinates");
$reponse = $pdo->query($stmt);
?>
<table border=1>
    <tr>
        <td>latitude</td>
    ...
    while ($data = $reponse->fetch()): ?>
        ...
        // AJOUT DE DATA DANS LE FICHIER JSON
        $ecritureBDD = ["rouge","vert",122,541];
        file_put_contents('location.json', json_encode($ecritureBDD));
        ?>
    <?php endwhile; ?>
    <!-- LECTURE DU FICHIER JSON -->
    <?php
    $lectureBDD = json_decode(file_get_contents('location.json'), true);
    print_r($lectureBDD);
    ?>
</table>
